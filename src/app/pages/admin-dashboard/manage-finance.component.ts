  ...
  template: `
    <div class="min-h-screen bg-slate-100 p-6">
      ...
      <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-5 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="md:col-span-2">
            ...
            <label for="transaction-image" class="inline-flex cursor-pointer items-center rounded-lg bg-slate-800 px-4 py-2 text-sm font-medium text-white hover:bg-slate-700">
              Choose image
            </label>
            <span class="ml-3 text-sm text-slate-500">{{ selectedFile?.name || 'No image selected' }}</span>
            <input id="transaction-image" type="file" accept="image/*" (change)="onFileSelected($event)" class="sr-only" />
          </div>
        </div>
      ...
      <details class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <summary class="cursor-pointer px-4 py-3 font-semibold text-slate-700">Transaction history</summary>
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
          ...
          <div class="space-y-3">
            ...
            <div>
              ...
              <label for="edit-transaction-image" class="inline-flex cursor-pointer items-center rounded-lg bg-slate-800 px-4 py-2 text-sm font-medium text-white hover:bg-slate-700">
                Choose image
              </label>
              <span class="ml-3 text-sm text-slate-500">{{ editFile?.name || 'No new image selected' }}</span>
              <input id="edit-transaction-image" type="file" accept="image/*" (change)="onEditFileSelected($event)" class="sr-only" />
